...
const Dashboard = () => {
    ...
    const onSearchChange = (event) => {
        ...
        let matched = currencies.filter(currency => currency.name.toLowerCase().includes(event.target.value.toLowerCase()));
    ...
    <main className='main'>
        <section>
            <div className="search-box">
                <input type="text" placeholder='search...' value={searchPhrase} onChange={onSearchChange} />
                {
                    suggestions.length > 0 && <ul className='suggestion-box'>
        ...
        <section>
            <div className="intervals">
                {Object.keys(INTERVALS).map(interval => (<button className={`interval-btn ${interval === ctx.interval && 'active'}`}
                onClick={() => {
        ...
        <section>
            <div className="title">
                <h4>Highest Market Cap Assests</h4>
            </div>
            <CurrencyTable />
        </section>
    </main>
